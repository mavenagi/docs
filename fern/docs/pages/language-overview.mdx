...
<Frame caption="Translate Co-Pilot Demo">
  <img src="../assets/copilot-translate.gif" alt="Translate Co-Pilot Demo"/>
</Frame>
...
Maven prioritizes the user’s preferred language. If a document is available in the user's language, it will be used; otherwise, the organization’s default language is used. Detecting document language is handled automatically, and duplicates across languages are not considered an issue, as it is assumed that translations are available in multiple languages. The system is designed to prevent duplicate document issues by filtering by user language first. Thus, deactivating foreign language documents is not necessary.
