...
<Steps>
  ...
  <Step title="Configure Maven Context Field">
    ...
    - Click **Save**
    - After saving, note the **Field Id** and **Shared Secret** which are displayed on the page below the **Integration ID** field.
  </Step>
...
- Set Actions
 ...
 {
  "webhook": {
    "id": "<REPLACE WITH_Webhook ID>" // REPLACE!!
  },
...
<Steps>
  <Step title="Create Front Application">
    - Navigate to [Front Developer
    Settings](https://app.frontapp.com/settings/developers) - Click **Create
    App** - Enter app name and description - Note the **App UID** and **App
    Secret** from Basic Information
  </Step>
  <Step title="Configure Channel">
    - Go to the **Features** tab - Click **Create channel type** - Configure the
    following settings: - Set **Webhook URL** to
    `https://chat.onmaven.app/api/front` - Set **Authentication strategy** to
    `Bearer` - Set **Contact type** to `Custom` - Set **Supported attachments**
    to `Text` - Enable **Reply/Compose** - Set **Composer type** to `Basic` -
    Set **Timeline Type UI** to `Chat Timeline` - Click **Create**
  </Step>
  <Step title="Connect Channel in Front">
    - Navigate to **Channels** under your workspace - Click **Connect a
    channel** - Find your channel under the "Other" section - Set **Name** and
    **Token** to `orgId-agentId` - Select target inbox and click **Continue**
  </Step>
</Steps>
#### Configuration
