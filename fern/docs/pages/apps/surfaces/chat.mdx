...
To correctly set the email address on the Zendesk contact, configure a Zendesk API email and API token.

By default, the Zendesk `external_id` is set to the user's `id` on `signedUserData`.
If your Zendesk `external_id` needs to be different, set the property `externalId` the new value on
the `signedUserData` property of the chat configuration.
If you need to set the contact email as the user ID, you should turn `setEmailAsExternalId` on by sending `true`. (This option only works for unsigned user data.)
