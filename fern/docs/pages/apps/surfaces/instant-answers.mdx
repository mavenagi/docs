...
1. In HubSpot, navigate to **Services** → **Knowledge Base** → **Settings** (cog icon)
...
```html
<!-- Load Maven AGI Instant Answers Script -->
<script src="https://instant-answers.onmaven.app/js/widget.js" async></script>
...
(function () {
  ...
  function isSearchPage() {
    ...
    return pathname.toLowerCase().includes("search") || /[?&](q|term|query)=/.test(search);
  }

  // Extract search query from URL
  function getSearchQuery() {
    const params = new URLSearchParams(window.location.search);
    return params.get("q") || params.get("term") || params.get("query") || "";
  }

  // Create and insert widget into page
  function insertMavenWidget() {

    // Parent container to insert the widget in. This must exist in your knowledge base
    const searchContainer =
      document.querySelector(".hs-search-results") ||
      document.querySelector("[class*='search']") ||
      document.querySelector("main") ||
      document.body;

    if (!searchContainer) {
      console.warn("[Maven] No container found");
      return;
    }

    const wrapper = document.createElement("div");
    wrapper.id = "maven-instant-answers-search";
    searchContainer.insertBefore(wrapper, searchContainer.firstChild);

    // Wait for DOM to fully update before initializing
    setTimeout(initializeMaven, 100);
  }

  // Initialize Maven widget and perform search
  function initializeMaven() {
    let attempts = 0;
    const maxAttempts = 30; // 3 seconds

    const checkMaven = setInterval(() => {
      attempts++;

      if (typeof Maven !== "undefined" && Maven.InstantAnswersWidget) {
        clearInterval(checkMaven);

        try {
          Maven.InstantAnswersWidget.init(MAVEN_CONFIG);

          const query = getSearchQuery();
          if (query) {
            Maven.InstantAnswersWidget.search({
              query: query,
              selector: "#maven-instant-answers-search"
            });
          }
        } catch (error) {
          console.error("[Maven] Error:", error);
        }
      }

      if (attempts >= maxAttempts) {
        clearInterval(checkMaven);
        console.warn("[Maven] Maven API not found");
      }
    }, 100);
  }
  ...
  function setup() {
    if (isSearchPage()) {
      // Wait for page to stabilize before inserting widget
      setTimeout(insertMavenWidget, 500);
    }
  }
...
```javascript
const MAVEN_CONFIG = {
  organizationId: "your-org-id",     // Your Maven organization ID
  agentId: "your-agent-id"           // Your Maven agent ID
};
```
