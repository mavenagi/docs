...
Note: If Maven does not render after adding this snippet, check the brower logs to see if either the query HTML element is missing or doesn't have a value. 
...
- Or the question may only be in the page url, and not in the HTML at all. In this case, try replacing the above `var text` line with
  `var text = new URLSearchParams(window.location.search).get("query");`. (Note: you may need to also comment out the if statement that checks 
